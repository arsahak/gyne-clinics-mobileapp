...
const CONDITIONS = [
  ...
  "Thyroid disorder", "Diabetes", "Hypertension", "None of the above",
];
const STEPS = ["Personal", "Cycle", "Conditions"];
...
// ── Pill selector ─────────────────────────────────────────────────────────────
function PillSelect({
  options, value, onChange, pink, pinkLite, colors,
}: {
...
// ── Multi-check ───────────────────────────────────────────────────────────────
function MultiCheck({
  options, values, onChange, pink, pinkLite, colors,
}: {
  options: string[]; values: string[]; onChange: (v: string[]) => void;
  pink: string; pinkLite: string; colors: any;
}) {
  const toggle = (o: string) => {
    if (o === "None of the above") { onChange(["None of the above"]); return; }
    const next = values.filter((x) => x !== "None of the above");
      ...
      {options.map((o) => {
        ...
        return (
          <Pressable key={o} onPress={() => toggle(o)}
            style={[mcS.row, { backgroundColor: sel ? pinkLite : colors.surface, borderColor: sel ? pink : colors.border }]}
          >
            <View style={[mcS.box, { backgroundColor: sel ? pink : "transparent", borderColor: sel ? pink : colors.border }]}>
              {sel && <CheckIcon color="#fff" />}
            </View>
            <Text style={{ fontSize: 14, color: sel ? pink : colors.text, fontWeight: sel ? "700" : "400" }}>{o}</Text>
          </Pressable>
        );
      })}
    </View>
...
const mcS = StyleSheet.create({
  row: { flexDirection: "row", alignItems: "center", gap: 12, borderRadius: 13, borderWidth: 1.5, paddingHorizontal: 14, paddingVertical: 12 },
...
export default function HealthProfileScreen() {
  ...
  const canNext = [
    name.trim().length > 1 && dob !== null && bloodType !== "",
    cycleLen !== "" && periodLen !== "",
    conditions.length > 0,
  ][step];
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : "height"}>
        ...
        <View style={[s.topBar, { borderBottomColor: colors.border }]}>
          ...
          <Text style={[s.topTitle, { color: colors.text }]}>
            {["Personal Info", "Cycle Info", "Health Conditions"][step]}
          </Text>
        ...
        >
          ...
          {step === 0 && (
            <>
              ...
              <Text style={[fS.label, { color: colors.text }]}>Blood Type</Text>
              <PillSelect options={BLOOD_TYPES} value={bloodType} onChange={setBloodType}
          ...
          {step === 1 && (
            <>
              ...
              <View style={[s.tipCard, { backgroundColor: pinkLite, borderColor: pink + "44" }]}>
                <HeartIcon color={pink} />
                <Text style={[s.tipText, { color: pink }]}>
                  If you're unsure, enter your best estimate. You can update this anytime.
                </Text>
              </View>
            </>
          )}
          ...
          {step === 2 && (
            <>
              <Text style={[s.stepSub, { color: colors.textMuted }]}>Select any conditions that apply to you</Text>
              <MultiCheck
                options={CONDITIONS}
                values={conditions}
              ...
              <View style={[s.tipCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                <Svg width={16} height={16} viewBox="0 0 24 24" fill="none">
                  <Path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"
                    stroke={purple} strokeWidth={1.8} strokeLinecap="round" />
                </Svg>
        ...
        <View style={[s.bottomBar, { backgroundColor: colors.background, borderTopColor: colors.border }]}>
          ...
          <Pressable
            onPress={next}
            disabled={!canNext}
            style={[s.ctaBtn, { backgroundColor: canNext ? pink : colors.border, opacity: canNext ? 1 : 0.55 }]}
          >
...
const s = StyleSheet.create({
  ...
  skipBtn:  { width: 44, height: 36, alignItems: "center", justifyContent: "center" },
